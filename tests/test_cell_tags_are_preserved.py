...
@pytest.mark.parametrize("fmt", formats_with_support_for_cell_metadata())
def test_tags_are_preserved(notebook_with_tags, fmt):
    text = writes(notebook_with_tags, fmt)
    nb = reads(text, fmt)
