...
def check_file_version(nbk, source_path, outputs_path):
    """Raise if file version in source file would override outputs"""
    _, ext = os.path.splitext(source_path)
    current = file_format_version(ext)
    version = nbk.metadata.get('nbrmd_format_version')
    if version:
        del nbk.metadata['nbrmd_format_version']

    # Missing version, still generated by nbrmd?
    if nbk.metadata and not version:
        version = current
