...
_ORG_KERNEL_DISPLAY_NAME_RE = re.compile(r"(?im)^#\+JUPYTER_KERNEL_DISPLAY_NAME:\s*(.+?)\s*$")
...
def org_to_notebook(text):
    ...
    text = _strip_org_targets(_strip_org_header_display_name(_strip_org_header_args(text)))
    ...
    _trim_trailing_newline_from_code_cells(notebook)
...
def _inject_kernel_header_args(
    ...
    Args:
        ...
        kernel_display_name: The Jupyter kernelspec display name.
    ...
    header_args_lines = []
    sanitized_display_name = _sanitize_keyword_value(kernel_display_name)
    if sanitized_display_name:
        header_args_lines.append(f"#+JUPYTER_KERNEL_DISPLAY_NAME: {sanitized_display_name}\n")
    header_args_lines.append(_format_header_args_line(block_language, {"kernel": kernel_name}))
    if extra_args:
...
def _sanitize_keyword_value(value: Any) -> Optional[str]:
    """Sanitize a top-level Org keyword value while preserving spaces."""
    if not value:
        return None
...
def _strip_org_header_display_name(text: str) -> str:
    """Remove the top-level kernel display-name keyword before Pandoc conversion."""
    return re.sub(r"(?im)^#\+JUPYTER_KERNEL_DISPLAY_NAME:\s*[^\n]*\n?", "", text)
...
def _org_metadata(text: str) -> dict[str, Any]:
    ...
    kernel_display_name_match = _ORG_KERNEL_DISPLAY_NAME_RE.search(text)
    ...
    if kernel_display_name_match:
        kernelspec["display_name"] = kernel_display_name_match.group(1).strip()
    elif "name" in kernelspec and "display_name" not in kernelspec:
...
def _trim_trailing_newline_from_code_cells(notebook) -> None:
    """Trim one trailing newline from each code cell produced by Pandoc."""
